...
lit supports UUID primary keys through dedicated insert functions.
...
When registering, lit detects that `Id` is not an integer and sets `HasIntId = false`. This changes how INSERT queries are generated.

## InsertUuid

Use this when you want lit to generate the UUID:
